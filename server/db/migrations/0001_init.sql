...
ALTER TABLE "users" ADD CONSTRAINT "users_age_check" CHECK ("age" IS NULL OR ("age" >= 0 AND "age" <= 120));--> statement-breakpoint
ALTER TABLE "module_prerequisites" ADD CONSTRAINT "module_prerequisites_no_self_check" CHECK ("module_id" <> "prerequisite_module_id");--> statement-breakpoint
ALTER TABLE "enrollments" ADD CONSTRAINT "enrollments_progress_check" CHECK ("progress" >= 0 AND "progress" <= 100);--> statement-breakpoint
ALTER TABLE "tutor_ratings" ADD CONSTRAINT "tutor_ratings_stars_check" CHECK ("stars" >= 1 AND "stars" <= 5);--> statement-breakpoint
ALTER TABLE "session_evaluations" ADD CONSTRAINT "session_evaluations_self_score_check" CHECK ("self_score" >= 1 AND "self_score" <= 5);--> statement-breakpoint
ALTER TABLE "growth_reports" ADD CONSTRAINT "growth_reports_period_check" CHECK ("period_start" <= "period_end");--> statement-breakpoint
ALTER TABLE "growth_reports" ADD CONSTRAINT "growth_reports_cognitive_score_check" CHECK ("axis_cognitive_score" >= 0 AND "axis_cognitive_score" <= 100);--> statement-breakpoint
ALTER TABLE "growth_reports" ADD CONSTRAINT "growth_reports_value_score_check" CHECK ("axis_value_score" >= 0 AND "axis_value_score" <= 100);--> statement-breakpoint
ALTER TABLE "growth_reports" ADD CONSTRAINT "growth_reports_time_score_check" CHECK ("axis_time_score" >= 0 AND "axis_time_score" <= 100);
