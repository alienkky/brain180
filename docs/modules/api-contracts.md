...
> Owner: 연다리 [통합설계] · 2026-05-30
> Day-1 Implementation: ALI-62 (schema) + ALI-66 (prompts) + ALI-67 (wire-up) + ALI-71 (QA)
...
| `service_unavailable` | 503 | Day-1 cut |
...
사이드 이펙트: User row 삽입(`status='pending_approval'`, `must_change_password=false`), 세션 쿠키 발급 (가입 직후 자동 로그인), Day-2+: 인증 메일 발송.
